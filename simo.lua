...
os.execute('chmod +x simo;chmod +x simo;./simo')
end
...
simo = Token:match("(%d+)")
os.execute('sudo rm -fr .CallBack-Bot/'..simo)
LuaTele = luatele.set_config{api_id=1846213,api_hash='c545c613b78f18a30744970910124d53',session_name=simo,token=Token}
function var(value)
...
function Dev(msg) 
if tonumber(msg.sender.user_id) == tonumber(Sudo_Id) or Redis:sismember(simo.."Dev",msg.sender.user_id) then
ok = true
else
ok = false
end
...
if tonumber(data.sender.user_id) == tonumber(simo) then
return false
end
end
function ChannelJoin(id)
JoinChannel = true
local chh = Redis:get(simo.."chfalse")
if chh then
...
if msg.sender.user_id == tonumber(simo) then
return false
end
if Redis:get(simo.."chsource") then
chsource = Redis:get(simo.."chsource")
else
chsource = "SSE_MO"
end
...
local Info_User = Redis:get(simo.."Twasl:UserId"..Message_Get.forward_info.date) or 46899864
if text == 'حظر' then
Redis:sadd(simo..'BaN:In:Tuasl',Info_User)  
return send(msg.chat_id,msg.id,Reply_Status(Info_User,'➢ تم حظره من الصانع').Reply,"md",true)  
end 
if text =='الغاء الحظر' or text =='الغاء حظر' then
Redis:srem(simo..'BaN:In:Tuasl',Info_User)  
return send(msg.chat_id,msg.id,Reply_Status(Info_User,'➢ تم الغاء حظره من الصانع ').Reply,"md",true)  
end 
end
end
if text == "➢ الغاء الامر" then
Redis:del(simo..msg.sender.user_id.."bottoken")
Redis:del(simo..msg.sender.user_id.."botuser")
Redis:del(simo..msg.sender.user_id.."make:bot")
...
Redis:set(simo..msg.sender.user_id.."run:bot",true)
send(msg.chat_id,msg.id,"➢ ارسل معرف البوت ليتم تشغيله")
return false 
end
---ايقاف البوتات
if text and Redis:get(simo..msg.sender.user_id.."stop:bot") then
Redis:del(simo..msg.sender.user_id.."stop:bot")
Redis:del(simo.."screen:on")
Redis:del(simo.."bots:folder")
userbot = text:gsub("@","")
for folder in io.popen('ls'):lines() do
...
Redis:set(simo..msg.sender.user_id.."stop:bot",true)
send(msg.chat_id,msg.id,"➢ ارسل معرف البوت ليتم ايقافه")
return false 
end
--الاشتراك الاجباري 
if Redis:get(simo.."ch:addd"..msg.sender.user_id) == "on" then
Redis:set(simo.."ch:addd"..msg.sender.user_id,"off")
...
Redis:del(simo.."chfalse")
Redis:set(simo.."chfalse",ch)
Redis:del(simo.."ch:admin")
Redis:set(simo.."ch:admin",da.result.invite_link)
else
send(msg.chat_id,msg.id,'➢ المعرف خطأ او البوت ليس مشرف في القناه ',"md",true)  
end
end
if text == "➢ تفعيل الاشتراك الاجباري" then
Redis:set(simo.."ch:addd"..msg.sender.user_id,"on")
...
if text and Redis:get(simo..msg.sender.user_id.."make:bot") == "devuser" then
...
local bottoken = Redis:get(simo..msg.sender.user_id.."bottoken")
local botuser = Redis:get(simo..msg.sender.user_id.."botuser")
...
Redis:sadd(simo.."userbots",botuser)
Redis:del(simo..msg.sender.user_id.."bottoken")
Redis:del(simo..msg.sender.user_id.."botuser")
Redis:del(simo..msg.sender.user_id.."make:bot")
...
if text and Redis:get(simo..msg.sender.user_id.."make:bot") == "token" then
...
if Redis:sismember(simo.."userbots",botuser) then
send(msg.chat_id,msg.id, "\n➢ عذرا هذا البوت مصنوع بالفعل","md",true)  
return false 
end 
Redis:set(simo..msg.sender.user_id.."botuser",botuser)
Redis:set(simo..msg.sender.user_id.."bottoken",text)
Redis:set(simo..msg.sender.user_id.."make:bot","devuser")
...
Redis:set(simo..msg.sender.user_id.."make:bot","token")
send(msg.chat_id,msg.id, "\n➢ ارسل توكن البوت الان","md",true)  
return false 
end 
----------end making
----broadcast all
if Redis:get(simo.."all:texting") then
if text == "الغاء" or text == '➢ الغاء الامر' then   
Redis:del(simo.."all:texting")
send(msg.chat_id,msg.id, "\n➢ تم الغاء الاذاعه","md",true)  
return false 
end 
Redis:set(simo.."3z:gp",text)
Redis:del(simo.."all:texting")
...
for k,v in pairs(list) do
https.request("https://api.telegram.org/bot"..bottoken.."/sendmessage?chat_id="..v.."&text="..URL.escape(Redis:get(simo.."3z:gp")).."&parse_mode=Markdown")
end
end
end
Redis:del(simo.."3z:gp")
Redis:del(simo.."all:texting")
send(msg.chat_id,msg.id,"➢ تم انتهاء الاذاعه في كل البوتات","html",true)
end
if Redis:get(simo.."all:texting:pv") then
if text == "الغاء" or text == '➢ الغاء الامر' then   
Redis:del(simo.."all:texting:pv")
send(msg.chat_id,msg.id, "\n➢ تم الغاء الاذاعه","md",true)  
return false 
end 
Redis:set(simo.."eza3a:pv",text)
Redis:del(simo.."all:texting:pv")
...
for k,v in pairs(list) do
https.request("https://api.telegram.org/bot"..bottoken.."/sendmessage?chat_id="..v.."&text="..URL.escape(Redis:get(simo.."eza3a:pv")).."&parse_mode=Markdown")
end
end
end
Redis:del(simo.."eza3a:pv")
Redis:del(simo.."all:texting:pv")
send(msg.chat_id,msg.id,"➢ تم انتهاء الاذاعه في كل البوتات","html",true)
end
if text == "➢ اذاعه عام للمجموعات" then
Redis:set(simo.."all:texting",true)
send(msg.chat_id,msg.id,"ارسل النص الان","html",true)
end
if text == "➢ اذاعه عام للمشتركين" then
Redis:set(simo.."all:texting:pv",true)
send(msg.chat_id,msg.id,"ارسل النص الان","html",true)
end
-------screen -ls
...
if text == "➢ فحص" then
Redis:del(simo.."All:pv:st")
Redis:del(simo.."All:gp:st")
for folder in io.popen('ls'):lines() do
if folder:match('@[%a%d_]') then
m = Redis:get(folder)
x = {m:match("(.*)&(.*)$(.*)+(.*)")}
...
list = Redis:smembers(bot_id.."ChekBotAdd") 
lt = Redis:smembers(bot_id.."Num:User:Pv") 
Redis:incrby(simo.."All:gp:st",tonumber(#list))
Redis:incrby(simo.."All:pv:st",tonumber(#lt))
end
end
send(msg.chat_id,msg.id,'\n✜ احصائيات جميع البوتات المصنوعه \n ✜ عدد المجموعات '..Redis:get(simo.."All:gp:st")..' مجموعه \n✜ عدد المشتركين '..Redis:get(simo.."All:pv:st")..' مشترك',"html",true)
end
-----ban all
...
-------delete 
if text and Redis:get(simo..msg.sender.user_id.."make:bot") == "del" then
if text == "الغاء" or text == '➢ الغاء الامر' then   
Redis:del(simo..msg.sender.user_id.."make:bot")
...
Redis:srem(simo.."userbots",text:gsub("@",""))
send(msg.chat_id,msg.id, "\n✜ تم حذف البوت بنجاح","md",true)  
return false 
end 
if text == "➢ حذف بوت" then
Redis:set(simo..msg.sender.user_id.."make:bot","del")
...
devbot = x[3]
bottoken = x[4]
Redis:sadd(simo..'3ddbots',botuser..' » '..devbot)
end
end
local list = Redis:smembers(simo..'3ddbots')
if #list <= 100 then
...
----end--3dd
if text and Redis:get(simo..msg.sender.user_id.."setchannel") then
if text == "الغاء" or text == '➢ الغاء الامر' then   
Redis:del(simo..msg.sender.user_id.."setchannel")
send(msg.chat_id,msg.id, "\n✜ تم الغاء تعيين قناه السورس","md",true)  
return false 
end 
if text:match("@(.*)") then
local ch = text:match("@(.*)")
Redis:set(simo.."chsource",ch)
...
Redis:set(simo..msg.sender.user_id.."setchannel",true)
...
Redis:set(simo.."twsl",true)
send(msg.chat_id,msg.id,"✜ تم تعطيل التواصل")
return false 
end
if text == "➢ الاحصائيات" then
local list = Redis:smembers(simo.."total")
...
if tonumber(simo) ~= tonumber(FilesJson.BotId) then
return send(msg_chat_id,msg_id,'✜ عذرا هاذا الملف غير مطابق مع البوت يرجى جلب النسخه الحقيقيه')
end -- end botid
...
Y = Y + 1
Redis:sadd(simo..'total',v)  
end
end
end
if text == "جلب نسخه" then
local UsersBot = Redis:smembers(simo.."total")
local Get_Json = '{"BotId": '..simo..','  
if #UsersBot ~= 0 then 
...
--brodcast
if Redis:get(simo..msg.sender.user_id.."brodcast") then 
if text == "الغاء" or text == '✜ الغاء الامر' then   
Redis:del(simo..msg.sender.user_id.."brodcast") 
send(msg.chat_id,msg.id, "\n✜ تم الغاء الاذاعه","md",true)  
return false 
end 
local list = Redis:smembers(simo.."total") 
...
if text == "➢ اذاعه" then
Redis:set(simo..msg.sender.user_id.."brodcast",true)
send(msg.chat_id,msg.id,"✜ ارسل الاذاعه الان")
return false 
end
---fwd
if Redis:get(simo..msg.sender.user_id.."brodcast:fwd") then 
if text == "الغاء" or text == '➢ الغاء الامر' then   
Redis:del(simo..msg.sender.user_id.."brodcast:fwd")
...
Redis:set(simo..msg.sender.user_id.."brodcast:fwd",true)
...
chinfo = Redis:get(simo.."ch:admin")
send(msg.chat_id,msg.id,'\n✜ عليك الاشتراك في قناة البوت لاستخذام الاوامر\n\n'..chinfo..'')
return false 
end
if not Redis:get(simo.."twsl") then
if msg.sender.user_id ~= tonumber(simo) then
if Redis:sismember(simo..'BaN:In:Tuasl',msg.sender.user_id) then
return false 
end
if msg.id then
Redis:setex(simo.."Twasl:UserId"..msg.date,172800,msg.sender.user_id)
...
if text and Redis:get(simo.."free:bot") then
return send(msg.chat_id,msg.id,"✜ الوضع المجاني معطل من قبل مطور الصانع")
end
if text == "/start" then
if not Redis:sismember(simo.."total",msg.sender.user_id) then
Redis:sadd(simo.."total",msg.sender.user_id)
end
reply_markup = LuaTele.replyMarkup{type = 'keyboard',resize = true,is_personal = true,
data = {
{
...
---making user
if text and Redis:get(simo..msg.sender.user_id.."make:bot") then
if text == "الغاء" then
Redis:del(simo..msg.sender.user_id.."make:bot")
...
Informationlua:close()
os.execute('cp -a ./source/. ./@'..botuser..' && cd @'..botuser..' && chmod +x * && screen -d -m -S '..botuser..' ./Run')
Redis:set(simo..msg.sender.user_id.."my:bot",botuser)
Redis:sadd(simo.."userbots",botuser)
Redis:del(simo..msg.sender.user_id.."make:bot")
...
if text == "صنع بوت" then
if Redis:get(simo..msg.sender.user_id.."my:bot") then
return send(msg.chat_id,msg.id,"✜ عفوا لديك بوت بالفعل احذفه اولا")
end
Redis:set(simo..msg.sender.user_id.."make:bot",true)
...
if Redis:get(simo..msg.sender.user_id.."my:bot") then
local botuser = Redis:get(simo..msg.sender.user_id.."my:bot")
os.execute("sudo rm -fr @"..botuser)
os.execute("screen -X -S "..botuser.." quit")
Redis:srem(simo.."userbots",botuser)
Redis:del(simo..msg.sender.user_id.."my:bot")
send(msg.chat_id,msg.id, "\n✜ تم حذف البوت بنجاح","md",true)  
else
...
if tonumber(data.message.sender.user_id) == tonumber(simo) then
return false
end
Run(data.message,data.message)
elseif data and data.luatele and data.luatele == "updateMessageEdited" then
local Message_Edit = LuaTele.getMessage(data.chat_id, data.message_id)
if Message_Edit.sender.user_id == simo then
return false
end
